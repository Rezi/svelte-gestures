...
> = (node: HTMLElement, inputParameters?: () => Partial<RotateParameters>) => {
	$effect(() => {
		const { gestureName, onMove, onDown, onUp, parameters } = rotateBase(node, inputParameters?.());
...
export const rotateComposition = (
	...
	const { onMove, onDown, onUp, parameters } = rotateBase(node, inputParameters);
...
function rotateBase(node: HTMLElement, inputParameters?: Partial<RotateParameters>) {
