...
> = (node: HTMLElement, inputParameters?: () => Partial<ShapeGestureParameters>) => {
	$effect(() => {
...
export const shapeGestureComposition = (
	...
	const { onMove, onDown, onUp, parameters } = shapeGestureBase(node, inputParameters);
	return {
		onMove,
		onDown,
		onUp,
...
function shapeGestureBase(node: HTMLElement, inputParameters?: Partial<ShapeGestureParameters>) {
