...
> = (node: HTMLElement, inputParameters?: () => Partial<PinchParameters>) => {
	$effect(() => {
		const { onMove, onDown, onUp, gestureName, parameters } = pinchBase(node, inputParameters?.());
...
export const pinchComposition = (node: HTMLElement, inputParameters?: Partial<PinchParameters>) => {
...
function pinchBase(node: HTMLElement, inputParameters?: Partial<PinchParameters>) {
