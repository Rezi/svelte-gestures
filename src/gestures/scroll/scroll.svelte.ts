...
function getScrollParent(node: HTMLElement, direction: Direction): HTMLElement | undefined {
	if (!node) {
		return undefined;
	}
...
> = (node: HTMLElement, inputParameters?: () => Partial<ScrollParameters>) => {
	$effect(() => {
		const { gestureName, onMove, onDown, onUp, parameters } = scrollBase(node, inputParameters?.());
...
export const scrollComposition = (
	...
	const { onMove, onDown, onUp, parameters } = scrollBase(node, inputParameters);
...
function scrollBase(node: HTMLElement, inputParameters?: Partial<ScrollParameters>) {
	...
	function scrollElementTo(el: HTMLElement | undefined, scrollValue: number, direction: Direction) {
		el?.scrollBy({
